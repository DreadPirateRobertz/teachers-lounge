...
//  1. Receives the current Unix timestamp (seconds, float) via ARGV[4]
...
//
// Time is supplied by Go via ARGV[4] rather than redis.call('TIME') so that
// tests can inject a fake clock without depending on miniredis FastForward
// affecting TIME inside Lua (which it does not in miniredis v2).
//
...
// It defaults to time.Now and may be replaced in tests to control time.
type Limiter struct {
	rdb    redis.Cmdable
	script *redis.Script
