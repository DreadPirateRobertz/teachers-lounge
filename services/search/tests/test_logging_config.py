"""Tests for search JSON structured logging configuration.
...
class TestConfigureLogging:
    ...
    def test_output_is_valid_json(self):
        ...
        logger, stream = self._capture_log("search")
    ...
    def test_log_level_filter_respected(self):
        ...
        logger, stream = self._capture_log("search", level="WARNING")
...
class TestTraceID:
    ...
    def test_trace_id_appears_in_log_output(self):
        ...
        stream = StringIO()
        configure_logging(service_name="search", log_level="DEBUG", stream=stream)
        tid = str(uuid.uuid4())
        token = set_trace_id(tid)
        try:
